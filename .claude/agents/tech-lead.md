...
1a. **Cheque o design system DO PROJETO.** Se o trabalho envolve UI, confirme que ESTE produto tem design system próprio: a seção **Design System** no `CLAUDE.md` e o arquivo `design/tokens.json` no repositório. **Cada produto tem o seu** — nunca reaproveite os tokens de outro projeto. Se NÃO existir, o **primeiro card** (etiqueta `senior`, dependência de todos os de UI) deve ser *"Criar design system do produto"* acionando o `design-engineer`; só depois venham as telas, que consomem esses tokens.
